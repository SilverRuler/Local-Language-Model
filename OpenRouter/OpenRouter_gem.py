...
# OpenRouter - OpenAI 호환(chat/completions) 엔드포인트
...
# OpenRouter 무료 모델 (Ox Alpha Free)
...
def get_api_key():
    """
    설정된 OpenRouter API Key를 반환합니다.
    """
    if not API_KEY:
        print()
        print("=" * 60)
        print("[ERROR] OpenRouter API Key가 설정되지 않았습니다.")
        print("=" * 60)
        sys.exit(1)
...
async def call_opencode_api_stream(
    ...
    """
    OpenRouter API에 요청하고 SSE 스트리밍으로 결과를 받습니다.
    """
    ...
    headers = {
        ...
        # 핵심:
        # dummy_key가 아니라 실제 OpenRouter API Key 사용
        "Authorization": f"Bearer {api_key}",
        "User-Agent": "SRT-Translator/1.0"
    }
